...
## 15. 档位选型实测（2026-08-14）

20 句中文（piper 合成，含中英混说与术语），RTX 4060 Laptop 8G，float16，
`initial_prompt` 与 `vad_filter` 按默认配置。逐句数据在 `ref/bench/*.json`
（`bench.py` 复现）。
...
| 档位 | 平均 CER | RTF | 显存 | 加载 |
|---|---|---|---|---|
| **qwen3**（GPU/fp16） | **8.8%** | **0.067** | ~3.9G | 5.5s |
...
| qwen3（无 GPU，CPU 回退） | 8.8% | 0.64–0.80 | — | 17–21s |

\* large-v3 被第 14 句大数字「一千二百三十四万五千六百七十八」单句异常拉高
（100% 全错，medium/qwen3 全对），去掉后约 14.9%，与 medium 同级。

> 无 GPU 时 qwen3 加载 17–21s，其中一大半是 onnxruntime 4 个 session 逐个尝试
> CUDA EP 失败再回退 CPU 的开销——Qwen3Asr 若在建 session 前探测 CUDA 可用性
> 可省回这段。CPU 的 RTF 对系统负载敏感（两次实测 0.69 / 0.80），典型约 0.64。

### 15.2 逐句 CER

| # | 句子 | qwen3 GPU | qwen3 CPU | large-v3 | turbo | medium | small |
|---|---|---|---|---|---|---|---|
| 1 | 今天天气很好，我们一起去公园散步吧。 | 0% | 0% | 0% | 0% | 0% | 6% |
| 2 | myna 是一个 Linux 桌面语音输入法… | 52% | 52% | 48% | 52% | 40% | 44% |
| 3 | 请把这段话粘贴到终端里，然后按回车执行。 | 22% | 22% | 11% | 28% | 22% | 28% |
| 4 | 识别准确率和推理速度之间需要权衡取舍。 | 0% | 0% | 0% | 0% | 0% | 17% |
| 5 | 我实验一下在火车上嘈杂的环境中能不能做。 | 5% | 5% | 0% | 0% | 0% | 5% |
| 6 | 我们明天下午三点在会议室开会… | 0% | 0% | 8% | 4% | 8% | 8% |
| 7 | 这本书定价五十八元，比网上便宜了不少。 | 0% | 0% | 18% | 41% | 29% | 18% |
| 8 | Python 和 Java 都是很流行的编程语言。 | 19% | 19% | 52% | 52% | 48% | 48% |
| 9 | 这个功能是通过 HTTP 接口实现的。 | 25% | 25% | 25% | 25% | 25% | 25% |
| 10 | 塞翁失马，焉知非福，这句话出自淮南子。 | 12% | 12% | 50% | 31% | 50% | 38% |
| 11 | 我叫王小明，来自北京，是一名软件工程师。 | 6% | 6% | 0% | 12% | 6% | 12% |
| 12 | 明天下雨的概率是百分之八十，出门记得带伞。 | 0% | 0% | 42% | 42% | 37% | 37% |
| 13 | 他反复强调，这件事绝对不能拖延… | 15%† | 15%† | 0% | 0% | 0% | 0% |
| 14 | 一千二百三十四万五千六百七十八。 | 0% | 0% | 100% | 100% | 0% | 100% |
| 15 | 请把音量调到百分之三十，然后开始录音。 | 0% | 0% | 29% | 29% | 29% | 29% |
| 16 | 我们的产品每两周迭代一次，发布时间定在星期五。 | 19% | 19% | 0% | 0% | 0% | 5% |
| 17 | 他说的是对的，但是我们还需要更多时间… | 0% | 0% | 0% | 0% | 0% | 4% |
| 18 | 从深圳到广州坐高铁只需要半个小时… | 0% | 0% | 0% | 0% | 0% | 0% |
| 19 | 这个文档已经保存到云盘了，你随时可以查看。 | 0% | 0% | 0% | 0% | 0% | 0% |
| 20 | 祝大家新年快乐，身体健康，万事如意。 | 0% | 0% | 0% | 0% | 0% | 7% |

† 第 13 句 qwen3 的 15% 是标点计错：识别文本多加了书名式引号（「他反复强调：
“这件事绝对不能拖延，必须今天完成。”」），CER 的 strip 只去标点不去引号。
语义是对的，不是真实错误。

### 15.3 关键句：各档位实际听成什么样

```
REF: Python 和 Java 都是很流行的编程语言。   （s8 中英混说）
  qwen3   Python和C都是很流行的编程语言。      ← 只错 Java→C
  large-v3 拜勒河道都是很流行的编程语言。        ← 整句崩
  medium  拜论和道都是很流行的编程语言。

REF: 塞翁失马，焉知非福，这句话出自淮南子。   （s10 成语古文）
  qwen3   塞翁失马焉知非福。这句话出自《淮南子》。   ← 全对
  large-v3 赛翁是马燕之肺腑这句话出自《淮南子》。
  medium  塞旺是马胭脂肺腑这句话出自怀男子。

REF: 明天下雨的概率是百分之八十，出门记得带伞。  （s12 百分比）
  qwen3   明天下雨的概率是百分之八十。出门记得带伞。  ← 汉字保留
  large-v3 明天下雨的概率是80%出门季的带散。          ← 缩写成 80% + 错字

REF: 一千二百三十四万五千六百七十八。        （s14 大数字）
  qwen3   一千二百三十四万五千六百七十八。    ← 汉字全对
  large-v3 12345678                          ← 数字对、汉字没输出，CER 计 100%

REF: 我们的产品每两周迭代一次，发布时间定在星期五。  （s16 长句）
  qwen3   我们的产品每两周迭代一次，发布时鉴定在性器物。 ← fp16 版退化（int8 版此句全对）
  large-v3 我们的产品每两周迭代一次发布时间定在星期五。
```

中英混说是所有模型的共同短板（s2 的 `myna`/`Linux` 全错、s9 的 `HTTP` 全错），
只有 `[hotwords]` / `initial_prompt` 能救。

### 15.4 GPU 化：为什么从 int8 换成 fp16

qwen3 档位最初接入的是 Daumee/Qwen3-ASR-0.6B-ONNX-CPU（**int8**，onnx 在
`onnx_models/` 子目录）。GPU 化实测撞上一堵墙：**int8 量化算子
（DynamicQuantizeLinear / MatMulInteger）在 CUDA 上没有 kernel**，onnxruntime
只能给 decoder 插 423 个 Memcpy 节点拷回 CPU 算，20 句里 decoder 占 81% 耗时——
GPU 化后 RTF 只有 0.40（CPU 0.709 → GPU 0.40，提速 44% 就到头了）。

换成原生 **fp16** 导出（cvxhull，onnx 在快照根目录）后：GPU RTF **0.067**
（再快 6 倍），CPU 回退 0.64–0.80（不输 int8 的 0.709）。准确率：fp16 与 int8
基本持平（8.8% vs 7.9%，0.9 个百分点几乎全来自第 16 句——int8 全对而 fp16 听成
「发布时鉴定在性器物」，fp16 低精度在个别长句上的代价）。综合速度与准确率，
fp16 全面替代 int8。实现：`qwen3_asr.py` 单/双 encoder 分派 + CUDA provider
探测；依赖 `onnxruntime-gpu` + cuDNN 因主盘紧张装 `/data`，systemd 服务与
run.sh 用 `PYTHONPATH`/`LD_LIBRARY_PATH` 指过去（详见
`ref/summary/models-and-deps.md`）。
...
1. **qwen3 又快又准，是默认**。CER 8.8% 甩开 whisper 各档一半以上：数字句
   ...
   （20 句 75 秒音频只转 5 秒），比 large-v3 快 3 倍多；无 GPU 回 CPU
   0.64–0.80，仍是准优先时最好选择。
...
4. **中英混说是共同短板**：`myna`/`Linux`/`Python`/`Java`/`HTTP` 全错。
   whisper 整句崩（「拜勒河道」），qwen3 相对好一点（Python 句只错一个词），
   但仍是 `[hotwords]` / `initial_prompt` 的职责范围。
5. **fp16 相对 int8 有一个可见代价**：个别长句（第 16 句）fp16 低精度会听错
   （「发布时鉴定在性器物」），int8 全对。整句级偶发，换来 GPU 6 倍速度，
   值得。
