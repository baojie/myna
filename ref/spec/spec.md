...
5 句中文（piper 合成，含中英混说与术语），RTX 4060 Laptop 8G，float16，
`initial_prompt` 与 `vad_filter` 按默认配置：

| 档位 | 平均 CER | RTF | 显存 | 加载 |
|---|---|---|---|---|
| **large-v3** | **13.2%** | 0.244 | 3838 MB | 5.6s |
...
1. **维持 large-v3 为默认**。它准得明显，而速度优势在交互上不可感知——
   一句 3~6 秒的话，large-v3 转写 0.87s、turbo 0.49s，用户察觉不到那 0.4 秒，
   但错字看得见。**为用户感知不到的速度牺牲看得见的准确率，是坏交易。**
2. **turbo 名不副实**（至少在中文上）。宣传是「接近 large-v3 的精度、快得多」，
   实测 CER 16.3% 甚至略逊于 medium 的 16.0%，还多占 160MB。它唯一的真实价值
   是比 large-v3 省 1.7GB 显存——当用户要同时跑别的 GPU 任务时，切 turbo
